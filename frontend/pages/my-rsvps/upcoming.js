...
export default function MyUpcomingRSVPs() {
  ...
  const [currentTimestamp, setEventStartTimestamp] = useState(new Date().getTime())
  ...
  return (
    <Dashboard page="rsvps" isUpcoming={true}>
      { account ? (
        <div>
          {data && !data.account && (<p>No Upcoming Event RSVPs found</p>)}

          {data && data.account && (
            <ul role="list" className="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-4 xl:gap-x-8">
              {data.account.rsvps.map((rsvp)=>{
                if (rsvp.event.eventStartTimestamp > currentTimestamp) {
                  return (
